...
const statusRank: Record<OrderStatus, number> = {
  ...
  [OrderStatus.PreProduction]: 2,
  [OrderStatus.InProduction]: 3,
  [OrderStatus.Shipped]: 4,
  [OrderStatus.OutForDelivery]: 5,
  [OrderStatus.Delivered]: 6,
  [OrderStatus.Cancelled]: 7,
};
const toStatusEnum = (raw: number | string): OrderStatus | undefined => {
  if (
    typeof raw === "string" &&
    Object.values(OrderStatus).includes(raw as OrderStatus)
  )
    return raw as OrderStatus;

  if (typeof raw === "number") {
    return (Object.keys(statusRank) as OrderStatus[]).find(
      (k) => statusRank[k] === raw
    );
  }
  return undefined;
};
const isAtLeast = (raw: number | string, compareTo: OrderStatus): boolean => {
  const current = toStatusEnum(raw);
  return current ? statusRank[current] >= statusRank[compareTo] : false;
};
...
const OrderDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : t("orderDetails.loadError")
        );
      } finally {
        setLoading(false);
      }
  ...
  const mapOrderToUiModel = useCallback(
    (order: UserOrder): OrderData => {
      // Check if order is cancelled
      const isCancelled =
        toStatusEnum(order.status) === OrderStatus.Cancelled ||
        order.isCanceled;
      ...
      if (isCancelled) {
        ...
        if (
          order.preProductionDate ||
          isAtLeast(order.status, OrderStatus.PreProduction)
        ) {
          steps.push({
            label: t("orderDetails.steps.preProduction"),
            date: order.preProductionDate
              ? formatDate(order.preProductionDate)
              : t("orderDetails.processing"),
            completed: isAtLeast(order.status, OrderStatus.PreProduction),
          });
        }

        // Add in-production step if applicable
        if (
          order.inProductionDate ||
          isAtLeast(order.status, OrderStatus.InProduction)
        ) {
          steps.push({
            label: t("orderDetails.steps.inProduction"),
            date: order.inProductionDate
              ? formatDate(order.inProductionDate)
              : t("orderDetails.processing"),
            completed: isAtLeast(order.status, OrderStatus.InProduction),
          });
        }

        // Add shipped step if applicable
        if (order.shippedDate || isAtLeast(order.status, OrderStatus.Shipped)) {
          steps.push({
            label: t("orderDetails.steps.shipped"),
            date:
              order.shippedDate && order.shippedDate !== "0001-01-01T00:00:00"
                ? formatDate(order.shippedDate)
                : t("orderDetails.processing"),
            completed: isAtLeast(order.status, OrderStatus.Shipped),
          });
        }
        ...
        if (
          ...
          isAtLeast(order.status, OrderStatus.OutForDelivery)
        ) {
          steps.push({
            label: t("orderDetails.steps.outForDelivery"),
            date:
              order.outForDeliveryDate &&
              order.outForDeliveryDate !== "0001-01-01T00:00:00"
                ? formatDate(order.outForDeliveryDate)
                : t("orderDetails.processing"),
            completed: isAtLeast(order.status, OrderStatus.OutForDelivery),
          });
        }
        ...
        steps.push({
          ...
          date:
            order.deliveredDate && order.deliveredDate !== "0001-01-01T00:00:00"
              ? formatDate(order.deliveredDate)
              : formatDate(order.estimadtedDelivereyDate),
          completed: toStatusEnum(order.status) === OrderStatus.Delivered,
        });
      }
    ...
    [t, isRTL, formatCurrency, formatDate, getCityName]
  );
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("orderDetails.cancelError")
      );
    } finally {
  ...
  const handleRefundOrder = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("orderDetails.refundError")
      );
    } finally {
  ...
  if (!orderData) {
    return (
      <div className="text-center mt-10 text-wine">
    ...
    <div className="container mx-auto mt-8 md:mt-16 px-4">
      ...
      <div className={`mx-2 md:mx-20 ${isRTL ? "rtl" : "ltr"}`}>
        <div className="flex flex-col w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-4 my-2 items-center">
            ...
            ) : orderData.steps.some(
                (step) => "isCancelled" in step && step.isCancelled
              ) ? (
              <button
                disabled
                className="bg-gray-400 text-mainColor font-playfair px-8 py-2 rounded-md text-lg md:text-xl cursor-not-allowed opacity-70"
            ...
            ) : orderData.steps.find(
                (step) => step.label === t("orderDetails.steps.delivered")
              )?.completed ? (
              // Check if any items can be refunded
              orderData.items.some(
                (item) => item.quantity > item.quantityRefunded
              ) ? (
          ...
          <div className="flex flex-wrap gap-4 w-full my-4 items-center border-b-2 border-ForthColor/50 pb-8">
            <p className="text-ForthColor font-Poppins text-xs md:text-xl font-medium">
              ...
              <span className="text-wine">{orderDate}</span>
            </p>

            {isCancelled ? (
              <>
                <span className="text-red-500 font-Poppins text-base md:text-xl font-medium">
                  |
                </span>
                <div className="flex items-center">
                  <svg
                    ...
                    >
                      <span
                        style={{
                          display: "block",
                          color:
                            "isCancelled" in step && step.isCancelled
                              ? "#E53935"
                              : index < activeStep
                              ? "#721013"
                              : index === activeStep
        ...
        <div className="mb-8 space-y-4">
          {items.map((item, index) => (
            <div
              key={index}
              className={`flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 border-b border-ForthColor/30 last:border-b-0 `}
            >
              ...
              >
                <p className="text-wine font-semibold text-base sm:text-lg">
                  {item.price}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            {delivery.flatNumber && delivery.floorNumber && (
              <p className="text-ForthColor text-base md:text-lg font-Poppins">
                {t("orderDetails.flat")} {delivery.flatNumber},{" "}
                {t("orderDetails.floor")} {delivery.floorNumber}
              </p>
            )}
            <p className="text-ForthColor text-base md:text-lg font-Poppins">
              {delivery.city}
      ...
      >
        <DialogTitle
          ...
          className="text-wine font-playfair"
        >
        ...
        <DialogContent>
          <DialogContentText
            ...
            className="text-ForthColor font-Poppins"
          >
      ...
      >
        ...
        <DialogContent>
          <DialogContentText
            id="refund-dialog-description"
            className="text-ForthColor font-Poppins mb-4"
          >
            ...
            {items.map((item, index) => {
              ...
              if (refundableQuantity <= 0) {
                return (
                  <div
                    key={index}
                    className="flex items-center border-b pb-4 opacity-50"
                  >
                    <div className="flex flex-1 items-center">
                      ...
                      <div className="flex-1">
                        <h4 className="font-semibold text-wine font-playfair">
                          {item.name}
                        </h4>
                        <p className="text-ForthColor text-sm font-Poppins">
                          {item.details}
                        </p>
              ...
              return (
                <div key={index} className="flex items-center border-b pb-4">
                  <Checkbox
                    ...
                    onChange={() =>
                      toggleItemSelection(itemId, refundableQuantity)
                    }
                    className="mr-2"
                  />
                  <div className="flex flex-1 items-center">
                    ...
                    <div className="flex-1">
                      ...
                      {item.quantityRefunded > 0 && (
                        <p className="text-amber-600 text-sm">
                          {t("orderDetails.partiallyRefunded", {
                            count: item.quantityRefunded,
                          })}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center">
                      <span className="text-ForthColor mr-2">
                      ...
                      <span className="text-ForthColor ml-2">
                        / {refundableQuantity}
                      </span>
                    </div>
                  </div>
